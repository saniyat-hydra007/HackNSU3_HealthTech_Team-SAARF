...
<br> <br> <br> <br>
<div style='margin-left:70px'><h4>Looking for Nurse?</h4>
<div style='margin-left:70px'>
    <form action="" method="POST">
        <fieldset>
            <label >Thana</label>
            <input  type="text" name="thana" placeholder='Mandatory'><br><br>
            <label>  District:</label>
            <input style='margin-left:30px'type="text" name="district" placeholder='Mandatory'><br><br>
            <label>  Gender:</label>
            <input style='margin-left:47px'type="text" name="gender" placeholder='Mandatory'><br><br>
            <label >Off_Day:</label>
            <input style='margin-left:55px' name="off_day" type="text" placeholder='Optional' /><br><br>
...
if(isset($_POST['search'])){
  ...
  $T=$_POST['off_day'];
  if($T==null){
  $query1 = "SELECT * FROM nurse WHERE thana='$BG'&& district='$D' && gender='$G'";
  }
  else{
  $query1 = "SELECT * FROM nurse WHERE thana='$BG' && district='$D'  && gender='$G' && off_day='$T'";
  }
  $result1=$conn->query($query1);
  echo "Nurse List:<br><br>";
  if($result1->num_rows>0){
    ?>
    <form method="POST">
    <form action="" method="POST">
    <input type="text"  name="name" size="15"value="<?php echo "name" ?>">
    <input type="text" name="phone_number"size="15" value="<?php echo 'phone_number' ?>">
    <input type="text" name="thana" size="15"value="<?php echo 'thana' ?>">
    <input type="text" name="district"size="15" value="<?php echo 'district' ?>">
    <input type="text" name="off_day"size="15" value="<?php echo 'off_day' ?>">
    <input type="text" name="wage_per_hour"size="15" value="<?php echo 'wage_per_hour' ?>">
    <input type="text"style=margin-bottom:30px name="gender" size="15"value="<?php echo 'gender'?>">  
    </form>
    <?php
    while($row=$result1->fetch_assoc()){
    ?>
    <form method="POST">
    <input type="text" name="name" size="15" value="<?php echo $row['name'] ?>">
    <input type="text" name="phone_number" size="15" value="<?php echo $row['phone_number'] ?>">
    ...
    <input type="text" name="off_day"size="15" value="<?php echo $row['off_day'] ?>">
    <input type="text" name="wage_per_hour"size="15" value="<?php echo $row['wage_per_hour'] ?>">
    <input type="text"style=margin-bottom:30px name="gender"size="15" value="<?php echo $row['gender']  ?>">
    </form>
    <?php
    }
  }
